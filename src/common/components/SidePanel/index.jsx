...
export default function SidePanel({navType}) {
    ...
    function handleRemoveSortingField(removingItem) {
        ...
        dispatch(
            addSortingField({
                id: currentNode.id,
                sorting: [
                    ...sorting.filter((item) => item.id !== removingItem.id),
                ],
            }),
        );
    }
    ...
    function handleRemoveNode() {
        dispatch(
            setStructure(
                currentReport?.structure?.filter(
                    ...
                    <div>
                        ...
                        {currentNode?.sorting?.map((item) => (
                            <SortingField
                                ...
                                options={currentNode?.columns?.map(
                                    (column) => ({
    ...
    function getNavMenuActions() {
        switch (navType) {
            ...
            case SIDE_PANEL_TYPES.BLOCK_MENU:
                return reportDesigner.reportsData.present.activeNodes.length ===
                    0
                    ? null
