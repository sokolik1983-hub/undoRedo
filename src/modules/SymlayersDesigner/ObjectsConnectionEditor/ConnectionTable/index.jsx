...
const ConnectionTable = ({
  ...
  useEffect(() => {
    setEnabledTables(
      tables
        .map((item) => {
          return { value: item.table_id.toString(), text: item.name };
        })
        .filter((item) => {
  ...
  return (
    <div className={styles.tableContent}>
      ...
      >
        {selectedTable &&
          selectedTable.columns.map((item) => {
            let res = null;
            ...
            return (
              <option value={item.field} key={item.field + item.type}>
                {item.field}
              </option>
            );
          })}
      ...
      <div className={styles.outsideConnectCheckbox}>
        <CheckBox
          ...
          label="Внешнее объединение"
          id={`outsideConnectionEnable+${tableName}`}
        />
      </div>
    </div>
  );
