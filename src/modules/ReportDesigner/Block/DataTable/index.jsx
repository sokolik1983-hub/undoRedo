...
function DataTable({
    ...
    function getSortedData() {
        if (currentReport?.dataset && structureItem?.sorting?.length > 0) {
            ...
            const sortArray = lodash.orderBy(
                result,
                sortFields,
                sortFieldsSort,
            );
            ...
            rows: [
                {
                    ...
                    values: lodash.keys(
                        lodash.groupBy(result, rowDimension.object.id),
                    ),
                },
            ],
        };
    ...
    function getGrouppedTableData(rowDimension, columnDimension, measure) {
        ...
        lodash.keys(gropp).forEach((key) => {
            const columnGroup = lodash.groupBy(
                result,
                columnDimension.object.id,
            );
