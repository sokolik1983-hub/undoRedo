...
const TableBody = ({
    ...
    useEffect(() => {
        if (displayMode === 'Data') {
            ...
            dispatch(
                getElementData(
                    {report_id: 'R1', element_id: bodyZone[0].id},
                    (res) => {
                        setIsFetching(false);
                        setResponse(res);
                    },
                ...
                headerField && (
                    <th onClick={() => handleClick('headerZone', headerField)}>
                        <Cell
                            ...
                            selected={
                                formattingElement &&
                                formattingElement.id === headerField.id
                            }
                        />
                    </th>
                )
                ...
                footerField && (
                    <th onClick={() => handleClick('footerZone', footerField)}>
                        <Cell
                            ...
                            selected={
                                formattingElement &&
                                formattingElement.id === footerField.id
                            }
                        />
                    </th>
                )
                ...
                <tr>
                    {zone?.cells?.map((item) => {
                        return (
                            <td
                                key={item.id}
                                onClick={() => handleClick('bodyZone', item)}
                            >
                                <Cell
                                    displayMode={displayMode}
                                    blockStyles={item.style}
