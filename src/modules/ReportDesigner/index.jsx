...
function ReportDesigner() {
    ...
    function handleKeyUp(event) {
        ...
        if (event.keyCode === 46 || event.keyCode === 8) {
            if (reportDesigner.reportsData.present.activeNodes.length > 0) {
                const activeNodeIds =
                    reportDesigner.reportsData.present.activeNodes.map(
                        (item) => item.id,
                    );
    ...
    const handleRenameReport = (repName) => {
        ...
        const reportIdx =
            reportDesigner?.reportsData?.present?.reports.indexOf(
                currentReport,
            );
        ...
        <div className={styles.root}>
            ...
            <div className={styles.wrapper}>
                {reportDesigner.reportsUi.ui.showFormulaEditor && (
                    <div
                        className={
                            activeTab === 1 ? formulaCompressed : styles.formula
                        }
                    >
                        <MiniFormulaIcon />
                        <textarea
                            ...
                            onKeyUp={(ev) => {
                                if (ev.key === 'Enter') {
                                    dispatch(
                                        setActiveNodeFormula(ev.target.value),
                                    );
                                    dispatch(setActiveNodes([]));
                                    dispatch(setConfigPanelVisible(false));
                                }
            ...
            <ReportFooter data={currentReport?.structure?.pgFooter} /> */}
                ...
                <div
                    className={
                        activeTab === 1 ? footerCompressed : styles.footer
                    }
                >
                    <div className={styles.tabs}>
                        {reportDesigner.reportsData.present.reports &&
                            reportDesigner.reportsData.present.reports.map(
                                (report) => {
                                    const isActive =
                                        reportDesigner.reportsData.present
