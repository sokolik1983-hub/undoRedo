...
const EditConnectorModal = ({visible, onClose}) => {
    ...
    const [testConnectionInputString, setTestConnectionInputString] =
        useState('');
    const [testConnectionInputLogin, setTestConnectionInputLogin] =
        useState('');
    const [testConnectionInputPassword, setTestConnectionInputPassword] =
        useState('');

    const types = useSelector(
        (state) => state.app.data.dictionaries.source_type,
    );
    ...
    useEffect(() => {
        if (connectorData.data) {
            ...
            setTestConnectionInputLogin(
                connectorData.data.fields.filter(
                    (field) => field.fieldKey === 'UID',
                )[0].value,
            );
            setTestConnectionInputPassword(
                connectorData.data.fields.filter(
                    (field) => field.fieldKey === 'PWD',
                )[0].value,
            );
        }
    }, [connectorData]);
            ...
            <div className={styles.connectionWrapper}>
                ...
                <Select
                    ...
                    options={sourceOptions?.filter(
                        (item) => item.value === connectType,
                    )} // Фильтурем для получения подходящих options в завимисомти от типо коннектора
