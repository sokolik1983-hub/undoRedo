...
import TestFailed from '../../../src/layout/assets/testFailedIcon.svg';
    ...
    const testConnection = (e) => {
        ...
        if (createConnectorForm.reportValidity()) {
            ...
            if (newConnector?.data?.fields[2]) {
                newConnector.data.fields[2].value =
                    newConnector?.data?.fields[2]?.value.toUpperCase();
            }
    ...
    useEffect(() => {
        if (creationResult?.result) {
            if (creationResult.result === 1) {
                dispatch(
                    showToast(TOAST_TYPE.SUCCESS, 'Соединение успешно создано'),
                );
            } else {
                dispatch(
                    showToast(TOAST_TYPE.DANGER, 'Ошибка создания соединения'),
                );
            }
        }
    }, [creationResult]);
                ...
                <div className={styles.connectionTypeSection}>
                    ...
                    <div className={styles.testConnectionWrapper}>
                        <div className={styles.gearsIconWrapper}>
                            {showTestOk && (
                                <TestOkIcon className={styles.testOkIcon} />
                            )}
                            {showTestFailed && (
    ...
    const createConnectorModalFooter = (
        <div className={styles.footerButtonsGroup}>
            <Button
                ...
                disabled={
                    newConnector?.data?.fields && connectorFields ? false : true
                }
            >
                Сохранить
            </Button>
