...
import { ReactComponent as Magnifier } from '../../../../layout/assets/reportObjectsPanelFiltersIcons/magnifier.svg';
...
const ReportObjectsPanelFilters = ({
  searchValue,
  setSearchValue,
}) => {
  ...
  const handleToggleIcon = (item) => {
    const newArr = arr.map(el => el.title === item.title ? 
    ...
    <div className={styles.root}>
      ...
      {showInput && (
        <>
          ...
          />
          <Magnifier className={styles.magnifier} onClick={() => setShowInput(!showInput)} />
        </>
)}
    </div>
  );
