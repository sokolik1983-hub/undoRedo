...
const Results = ({title, isQueryExecute, onQueryTextCreate, onObjFilEdit}) => {
    ...
    const symLayerDataFromQuery = useSelector(
        (state) => state.app?.data?.symanticLayerQueryResult,
    );
    const fetchingDataResult = useSelector(
        (state) => state.app?.data?.queryResult,
    );
    ...
    useEffect(() => {
        if (symLayerDataFromQuery && isLoading)
            startFetchingData(symLayerDataFromQuery.id);
