...
const ResultsTable = ({ headersArr, bodyArr }) => {
  ...
  //   field.name = `${field.name.match(/\.[a-z0-9_]+$/gi)}`.slice(1);
  //   return field;
  // });
          ...
          <tr>
            {headersArr &&
              headersArr.map(column => (
                <th key={column.id} id={column.id} order={column.order}>
                  {column.name}
                </th>
              ))}
          </tr>
...
ResultsTable.propTypes = {
  ...
  bodyArr: PropTypes.arrayOf(oneOfType([PropTypes.string, PropTypes.number]))
    .isRequired
};
